...
// Laravel Server
const String server = "http://10.205.114.144/rfid-lab/public/index.php/api/v1";
const String deviceId = "LAB-TKJ-sukur"; // pembeda lab atau device
...
void loop() {
  ...
  for (byte i = 0; i < rfid.uid.size; i++) {
    if (rfid.uid.uidByte[i] < 0x10) TAG += "0";  // Tambah 0 depan jika < 0x10
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String urlAbsen = server + "/absen?uid=" + TAG + "&device_id=" + deviceId;
    ...
    if (codeAbsen > 0) {
      ...
      lcd.clear();
      if (resAbsen.indexOf("MASUK OK") >= 0) {
  lcd.setCursor(0, 0);
  lcd.print("Absen Masuk");
  lcd.setCursor(0, 1);
  lcd.print("Berhasil");
} else if (resAbsen.indexOf("KELUAR OK") >= 0) {
  lcd.setCursor(0, 0);
  ...
  } else if (resAbsen.indexOf("TIDAK ADA JADWAL") >= 0) {
        lcd.setCursor(0, 0);
        lcd.print("Tidak Ada");
        lcd.setCursor(0, 1);
        lcd.print("Jadwal Saat Ini");
} else {
  lcd.setCursor(0, 0);
